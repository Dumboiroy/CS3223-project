...
public class Parser {
	...
	public QueryData query() {
		...
		return new QueryData(fields, tables, pred, sortFields, sortAscending, groupFields, aggregateFunctions);
	}
...
//   }
	...
	/**
	 * Parses the SELECT clause and separates regular fields from aggregate
	 * functions. Syntax: field | AGGFN(field) [, field | AGGFN(field)]* Example:
	 * "dept, COUNT(id), SUM(salary)"
	 * 
	 * @param aggregateFunctions list to accumulate aggregate function strings found
	 *                           in SELECT
	...
	private SelectClauseInfo selectListWithAggregates() {
		...
		while (true) {
			if (checkAggregateFn()) {
				...
				parseAggregateFn(fields, aggregateFunctions);
				// fields now has ALL fields in SELECT clause, aggFns has the aggFns.
			} else {
				// Parse regular field
	...
	/**
	 * Parses the fields in a GROUP BY clause. Syntax: field [, field]* Example:
	 * "dept, role"
	 * 
	 * Note: Aggregate functions are NOT parsed here; they come from the SELECT
	 * list.
	 * 
	...
	/**
	 * Checks if the current keyword is an aggregate function. Used in
	 * selectListWithAggregates() to check if current field is aggregated.
	 * 
	 * @return the aggregate function string (e.g., "count(id)", "sum(salary)")
	 */
	private boolean checkAggregateFn() {
		return (lex.matchKeyword("count") || lex.matchKeyword("sum") || lex.matchKeyword("max")
				|| lex.matchKeyword("min") || lex.matchKeyword("avg"));
	}

	/**
	 * Parses a single aggregate function invocation. Syntax: AGGREGATEFN(fieldname)
	 * Supported functions: COUNT, SUM, MAX, MIN, AVG
	 * 
	 * @return the aggregate function string (e.g., "count(id)", "sum(salary)")
	 */
	private void parseAggregateFn(List<String> fields, List<String> aggFns) {
		String fn = "";
		...
		String fieldName = field();
		lex.eatDelim(')');
		// add the field name to the list of fields in select clause
		fields.add(fieldName);
		// then add the aggFns to the list of aggFns
	 ...
	 *
	 * Each item has the following form: field [ASC | DESC | null]
	 *
	 * ASC is used by default when no direction is specified.
	 */
	private void orderList(List<String> sortFields, List<Boolean> sortAscending) {
		while (true) {
